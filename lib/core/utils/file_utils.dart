...
class FileUtils {
  ...
  static Future<File> imageToFile({required String imagePath, String imageName = 'temp'}) async {
    var bytes = await rootBundle.load(imagePath);

    String tempPath = (await getDownloadsDirectory() ?? await getApplicationCacheDirectory()).path;
  ...
  static Future<String> savePhotoInCacheFromSystemPath(String systemPath) async {
    var cachePath = await getPlatformStoragePath();
    var newPath = "$cachePath/${getNameFromPath(systemPath)}";
    var file = File(systemPath);
    await file.copy(newPath);
    logD("FileUtils:savePhotoInCacheFromSystemPath: copy $systemPath to $newPath");
    return newPath;
  }

  static Future<File> savePhotoInCacheFromUrl(String url) async {
